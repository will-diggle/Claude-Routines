"""
Minimal, standalone A2 test. No fixtures, no factbase, no pipeline stages -- just two
hardcoded native articles (German, French) rewritten straight to A2, printed to stdout.

Mirror of test_a1_de_fr.py, same source articles, level swapped to A2. GRAMMAR_RULE_A1 is
deliberately scoped to A1 only -- A2 legitimately allows the imperfect/simple past and
basic subordinate clauses, so this run has no grammar constraint beyond GLOSS_RULE_BEGINNER
and ATTRIBUTION_RULE_BEGINNER (both still apply at A2). Point of this test is to see
whether A2, without the strict A1 constraint, already lands where it should.
...
def run(lang: str, article: dict) -> None:
    prompt = W.build_rewrite_prompt(lang, "A2", "longer", article)
    print(f"\n{'=' * 20} {lang.upper()} A2 — PROMPT SENT {'=' * 20}\n")
    print(prompt)

    client = genai.Client()
    raw, finish = W.call_gemini(
        client, W.MODEL_2S, prompt, f"test/{lang}-A2", schema=W._SCHEMA_ARTICLE,
    ...
    print(f"\n{'=' * 20} {lang.upper()} A2 — RESULT ({len(body.split())} words) {'=' * 20}\n")
    print(body)
